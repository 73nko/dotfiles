...
# Use the stable symlink (e.g. /usr/local/bin/fish) instead of `status fish-path`,
# which resolves to a versioned Cellar path that disappears on `brew upgrade fish`
# and breaks every running session's prompt.
command -v fish | read -l fish_path
